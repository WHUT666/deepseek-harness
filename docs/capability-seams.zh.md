...
flowchart LR
  ...
  pkg_fluent["fluent"]
  svc_fluent["ctx.fluent<br/>ANSYS Fluent batch seam"]
  pkg_tool_fluent["tool-fluent"]
  ...
  pkg_fluent --> svc_fluent
  pkg_fluent_local --> svc_fluent
  pkg_fs --> svc_fs
  pkg_fs_e2b --> svc_fs
  pkg_fs_local --> svc_fs
  ...
  svc_subprocess --> pkg_fluent_local
...
| `ctx.subprocess` | `seam` | [`subprocess`](../packages/subprocess/subprocess) | [`subprocess-local`](../packages/subprocess/subprocess-local), [`subprocess-e2b`](../packages/e2b/subprocess-e2b) | [`bash-local`](../packages/shell/bash-local), [`bash-sandbox`](../packages/shell/bash-sandbox), [`terminal-bash`](../packages/terminal/terminal-bash), [`lsp-stdio`](../packages/lsp/lsp-stdio), [`fluent-local`](../packages/fluent/fluent-local), [`subagent-acp`](../packages/subagent/subagent-acp), [`subagent-codex`](../packages/subagent/subagent-codex), [`subagent-claude-code`](../packages/subagent/subagent-claude-code) | - | Bash 执行器、PTY shell 后端、LSP Host、Fluent 本地后端，以及进程外 ACP、Codex 和 Claude Code subagent 后端都通过 ctx.subprocess 执行 spawn；该服务负责进程坐标、进程树／会话生命周期、stdio 处置、终端机制和 kill 升级。 |
...
| `ctx.fluent` | `seam` | [`fluent`](../packages/fluent/fluent) | [`fluent-local`](../packages/fluent/fluent-local) | [`tool-fluent`](../packages/fluent/tool-fluent) | - | 提供方注册与选择，加上标准化的 probe／runJournal 执行；该 seam 不提供求解器 CLI 逃生口，后端必须转换为标准化请求和结果。 |
