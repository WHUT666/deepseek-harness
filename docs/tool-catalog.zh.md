...
| `@deepseek-ai/dsh-tool-fluent` | `fluent` | `ctx.tools`、`ctx.fluent`、`ctx.systemPrompt`、`ctx.jobs at call time for run_in_background` | `tool/call`、`tool/result` | - | fluent 工具将提供方选择和求解器子进程置于 ctx.fluent 之后，因此其模型可见 schema 在更换提供方时保持稳定。运行时要求已注册提供方，例如 `@deepseek-ai/dsh-fluent-local`；如果没有提供方，调用会返回结构化 `FLUENT_PROVIDER_UNAVAILABLE` 错误，而不会改变 schema。`run_in_background` 运行会注册到通用 `ctx.jobs` 运行时，并通过 `@deepseek-ai/dsh-tool-jobs` 的 `job_*` 工具收集／停止；`enableRunInBackground` 配置（默认 true）在禁用时会完全移除该参数。 |
...
探测本地 ANSYS Fluent 安装，或以批量方式运行一份 Scheme/TUI journal。operation 可取 probe 或 runJournal。runJournal 要求 journal_path。dimension 可选（2d、3d、2ddp、3ddp）。processors 是可选的并行进程数。长 iterate／solve 必须设置 run_in_background: true：调用立即返回 job id；用 job_output 读取输出，用 job_kill 停止。
...
来源：[`packages/fluent/tool-fluent/src/index.ts`](../packages/fluent/tool-fluent/src/index.ts)

fluent 工具将提供方选择和求解器子进程置于 ctx.fluent 之后，因此其模型可见 schema 在更换提供方时保持稳定。运行时要求已注册提供方，例如 `@deepseek-ai/dsh-fluent-local`；如果没有提供方，调用会返回结构化 `FLUENT_PROVIDER_UNAVAILABLE` 错误，而不会改变 schema。`run_in_background` 运行会注册到通用 `ctx.jobs` 运行时，并通过 `@deepseek-ai/dsh-tool-jobs` 的 `job_*` 工具收集／停止；`enableRunInBackground` 配置（默认 true）在禁用时会完全移除该参数。
