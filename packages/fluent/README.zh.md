...
Service Definition 位于 `fluent/fluent/`。该 seam 恰好公开两种操作：`probe` 与 `runJournal`，并另有 `startJournal` 用于活 journal 句柄。没有自由形式的求解器 CLI 逃生口；因此，替换提供方不会改变模型请求版本检查或 journal 运行的方式。
